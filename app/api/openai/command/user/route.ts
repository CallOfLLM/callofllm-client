...
- 방향만 말하고 거리를 말하지 않으면 그 방향의 맵 끝을 목적지로 잡으세요. 스쿼드는 정지 명령을 받거나 목적지에 닿을 때까지 계속 나아갑니다.
  - 앞이면 destinationX는 6399, destinationY는 기준점의 posY 평균 그대로
  - 뒤면 destinationX는 0, destinationY는 기준점의 posY 평균 그대로
  - 우측이면 destinationY는 3199, destinationX는 기준점의 posX 평균 그대로
  - 좌측이면 destinationY는 0, destinationX는 기준점의 posX 평균 그대로
- 사용자가 거리를 말하면 맵 끝 대신 기준점에 그만큼 더한 좌표를 쓰세요. 조금, 살짝은 250이고 크게, 멀리, 깊숙이는 1000이며 숫자를 직접 말하면 그 값을 씁니다.
- 여러 방향을 함께 말하면 각 축을 따로 계산하세요. 예를 들어 거리 없이 앞으로 그리고 우측으로면 destinationX는 6399, destinationY는 3199입니다.
- 계산 결과가 맵을 벗어나면 X는 0..6399, Y는 0..3199 안으로 잘라서 사용하세요.
- 이렇게 만든 명령은 계속 전진하는 명령이므로 message에서 정지 명령을 내리기 전까지 나아간다는 점을 알려주세요.

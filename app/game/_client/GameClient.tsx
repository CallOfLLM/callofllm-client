...
// 외부 서버 주소는 배포 환경마다 달라진다. 만료되는 Quick Tunnel 주소를 코드에
// 고정하지 않고 NEXT_PUBLIC_GAME_WS_URL로 주입하며, 값이 없으면 상단 입력창에서 받는다.
...
const EXPECTED_MAP_VERSION = 1;
const EXPECTED_GRID_CELL_SIZE = 10;
...
export default function GameClient() {
  ...
  // 기본 주소가 없으면 즉시 주소 입력 안내를 보여 주고, 주소가 있으면 첫 자동 연결 결과를 기다린다.
  ...
  // CREATE 전체 완료와 서버의 RUNNING 전환을 모두 확인해야 부분 배치 상태에서 브리핑이 열리지 않는다.
  const networkReady =
    protocolReady &&
    setupPhase === "ready" &&
  ...
  const pushLog = useCallback((level: LogLevel, text: string) => {
    ...
    // 복구 가능한 게임/소켓 오류를 console.error로 쓰면 Next 개발 오버레이가
    // 애플리케이션 예외처럼 소스 프레임을 띄운다. 화면 로그 레벨은 유지하되 콘솔은 경고로 남긴다.
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      console.warn("[WS] 생성 실패 — URL 형식을 확인하세요:", url, err);
    ...
    ws.onmessage = (e) => {
      ...
      switch (packet.pktType) {
        ...
        case PKT.SC_COMMAND_RESULT: {
          ...
          switch (packet.requestPacketType) {
            ...
            case PKT.CS_SELECT_MAP:
              ...
              // WELCOME 직후 오는 기본 MAP_INFO를 SELECT_MAP 확인으로 재사용하지 않는다.
              // V15 서버는 이 Type 104 다음에 선택 결과 Type 106을 보낸다.
        ...
        case PKT.SC_MAP_INFO: {
          ...
          const setup = setupRef.current;
          // WELCOME 직후의 기본 MAP_INFO는 표시만 한다. SELECT_MAP의 OK를 받은 뒤 도착한
          // 동일 mapID의 새 MAP_INFO만 다음 단계의 인과적 확인으로 인정한다.
          if (setup.phase === "selectingMap" && setup.mapCommandAccepted && packet.mapID === setup.mapID) {
            const metadataMatches =
              packet.mapVersion === EXPECTED_MAP_VERSION &&
              packet.worldWidth === MAP_W &&
              packet.worldHeight === MAP_H &&
              packet.gridCellSize === EXPECTED_GRID_CELL_SIZE;
            if (!metadataMatches) {
              failSetup(
    ...
    ws.onerror = (e) => {
      console.warn(`[WS] ERROR (${elapsed()}) — 브라우저는 보안상 상세 사유를 주지 않습니다. 바로 뒤에 오는 CLOSE 코드를 보세요.`, e);
  ...
  useEffect(() => {
    // 환경변수가 없는 개발 환경에서는 빈 주소로 자동 접속하지 않고 사용자가
    // 화면 상단에 실제 외부 V15 서버 주소를 입력할 때까지 기다린다.
    if (!DEFAULT_WS_URL) return;

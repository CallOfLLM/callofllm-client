          ...
          {stages.map((stage) => {
            ...
            return stage.available ? (
              <Link
                ...
                className="min-h-48 rounded-xl border border-white/10 bg-white/5 p-5 transition hover:-translate-y-1 hover:border-sky-400/50 hover:bg-sky-400/10"
              >
                {content}
              </Link>
            ) : (
              <article key={stage.id} className="min-h-48 cursor-not-allowed rounded-xl border border-white/5 bg-white/[0.02] p-5">
                {content}
              </article>
            );
          })}
